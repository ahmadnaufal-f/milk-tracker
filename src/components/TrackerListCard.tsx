...
// import { format, addDays, subDays } from 'date-fns'; // Removed
...
const TrackerListCard: React.FC<TrackerListProps> = ({ date, showViewMore = false, onDateChange, title = "Daily Log" }) => {
  ...
  return (
    <Card className="w-full gap-4 shadow-md overflow-hidden border-solid border-purple-300 bg-purple-50  rounded-3xl">
      <div className="flex items-center justify-center px-8">
        <h3 className="text-lg font-medium mb-2 text-start text-purple-900 flex items-center justify-center gap-2">
          {onDateChange ? (
            title
          ) : (
            <>
              <span className="text-purple-600">✨</span> Today's Pumping <span className="text-purple-600">✨</span>
            </>
          )}
        </h3>
      </div>
      ...
      {onDateChange && sessions.length > 0 && (
        <div className="mx-4 p-4 bg-purple-100/50 rounded-2xl grid grid-cols-2 gap-4 border border-purple-100">
...
export default TrackerListCard;
